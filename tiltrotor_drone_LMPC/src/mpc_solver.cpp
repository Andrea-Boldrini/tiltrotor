...
//MX EulerRatesTransformation(const MX& phi, const MX& theta) {
//    MX T = MX::eye(3);
//    
//    T(0, 0) = 1;
...
MPCSolver::MPCSolver() {
    ...
    //MX T_matrix = EulerRatesTransformation(x(6), x(8));
    //MX euler_rates = mtimes(T_matrix, MX::vertcat({x(7), x(9), x(11)}));
    ...
    for (int i = 0; i < Np_; ++i) {
        ...
        g.push_back(X(6));
        lbg_vec.push_back(-3.1416/180*30);
        ubg_vec.push_back(3.1416/180*30);
        
        //g.push_back(X(0));  // To handle disturbance
    ...
    // Input constraints
    DM u_min = DM::vertcat({-20.0, -40.0, -16.0, -16.0, -8.0});
    DM u_max = DM::vertcat({ 20.0,  40.0,  16.0,  16.0,  8.0});
