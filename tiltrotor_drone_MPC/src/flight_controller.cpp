...
void FlightController::sendCmds()
{
    ...
    Eigen::Vector3d torques(_torques[0], _torques[1], _torques[2]);
    
    //_wrench_published = false;
    if (!_wrench_published) {
        _dis_state = std::vector<double>(6 * 15, 0.0);
    }   
    std::pair<Eigen::VectorXd, double> dis = wrench_estimator.computeWrench(attitude, velocity, rates, _T_tot, torques, _tilt);
    ...
    _f.x = MPC[0] - _m*9.81*sin_pitch;
    _f.z = MPC[1] + _m*9.81*cos_pitch*cos_roll;
    _tilt = -atan(_f.x / _f.z);
    _T = (sqrt(pow(_f.x, 2) + pow(_f.z, 2)))/(2*9.81*_m*0.817);
