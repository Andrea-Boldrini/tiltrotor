...
MPCSolver::MPCSolver() {
    ...
    x_dot(Slice(0, nx_, 2)) = x(Slice(1, nx_, 2));

    // Dynamics equations
    SX R = rotationMatrixFromEuler(x(6), x(8), x(10));
    SX dis = d(Slice(0, 3));
    ...
    SX F = (1.0 / mass_) * SX::mtimes(R, (SX::vertcat({u(0), 0, u(1)}) + forces + dis)) - g_ * SX::vertcat({0, 0, 1});
    x_dot(Slice(1, nx_/2, 2)) = F;

    // Angular dynamics
    SX rates = x(Slice(7, nx_, 2));
    SX torque = u(Slice(2, 5));
    SX ang_accel = SX::mtimes(SX::inv(I), (torque - SX::cross(rates, SX::mtimes(I, rates))));
    x_dot(Slice(7, nx_, 2)) = ang_accel + SX::mtimes(SX::inv(I), d(Slice(3, 6)));
    ...
    SX cost = 0;
    SX Q_cost = SX::diag(SX::vertcat({8, 1, 3, 1, 3, 0.8, 1, 0.5, 1.5, 1, 1.5, 1}));
    SX R_cost = SX::diag(SX::vertcat({0.1, 0.1, 0.01, 0.01, 0.05}));
    SX X = x0;
    for (int i = 0; i < Np_; ++i) {
    ...
    SX U_flat = vertcat(U);
    lbx_ = DM::repmat(DM::vertcat({-20, -20, -2, -2, -2}), Nc_, 1);
    ubx_ = DM::repmat(DM::vertcat({20, 20, 2, 2, 2}), Nc_, 1);

    // Solver definition
    SXDict nlp = {{"x", U_flat}, {"f", cost}, {"p", P}};
    Dict opts;
    opts["qpsol"] = "qpoases";    
