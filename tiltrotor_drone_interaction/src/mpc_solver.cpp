...
MPCSolver::MPCSolver() {
    ...
    nh.param<int>("/flight_controller/Nc", Nc_, 6);
    nh.param<double>("/flight_controller/Ts", Ts_, 0.05);
    nh.param<bool>("/flight_controller/grasp_enabled", grasp_enabled, true);
    // in the current architecture the parameters used are the default one and cannot be read from "/flight_controller/..."
    
    buildSolver();
}


void MPCSolver::buildSolver() {
    // Physical parameters
    mass_ = 5.1664;
    g_ = 9.8066;
    I_x_ = 0.026;
    ...
    SX p_tree = SX::vertcat({0.5, 0.5, 1});
...
std::pair<std::vector<double>, double> MPCSolver::solve(const std::vector<double>& x_vec,
    ...
    return {std::vector<double>(u0->begin(), u0->end()), grasp_enabled}; // Second output is for debug
}
