...
void FlightController::cbReconfig(tiltrotor_drone::flight_controllerConfig &config, uint32_t level)
{
    ...
    _grasp_flag = config.grasp_enabled;
}
...
void FlightController::wrenchCallback(const geometry_msgs::WrenchStamped::ConstPtr& msg)
{
    ...
    _dis_state.insert(_dis_state.end(), _commanded_wrench_msg.wrench.torque.z);
}
...
void FlightController::initRosComms(ros::NodeHandle *nh)
{
    ...
    nh->advertise<std_msgs::Float64MultiArray>("mavros/vision_pose/velocity", 1);
    
    _tilt_pub = nh->advertise<std_msgs::Float64>("tiltrotor/angle_setpoint", 1);    
    
    _wrench_pub = nh->advertise<std_msgs::Float64MultiArray>("estimated_wrenches", 1);
    ...
    _end_effector_pub = nh->advertise<geometry_msgs::Point>("/end_effector_position", 1);
    ...
    _velocity_sub = nh->subscribe<geometry_msgs::TwistStamped>("/mavros/local_position/velocity_body", 1, &FlightController::velocityCallback, this);
...
void FlightController::sendCurrentDronePose()
{
    if (_drone_name_received)
    {
        ...
        std::array<std::array<double, 3>, 3> R_matrix = Inertial2BodyRotationMatrix(_current.roll_rad, 0, _current.yaw_rad);
        std::array<double, 3> _current_rates_body = multiplyMatrixVector(R_matrix, _current_rates_global);
        
        //std::array<double, 3> _current_rates_body = {roll_rate, pitch_rate, yaw_rate};
        
        _old.p = _current.p;
        _current.p = _current_rates_body[0];
        _old.q = _current.q;
        _current.q = _current_rates_body[1];
        _old.r = _current.r;
        _current.r = _current_rates_body[2];
        ...
        end_effector_position = addVectors(end_effector_position1, position_uav_vehicle);
...
void FlightController::sendCmds()
{
    ...
    if (!_wrench_published) {
        _dis_state = std::vector<double>(6, 0.0);
    }   
    std::pair<Eigen::VectorXd, Eigen::Vector3d> dis = wrench_estimator.computeWrench(attitude, velocity, rates, _T_tot, rotors_thrust, _tilt, _m, _a, _tau_filter, _I.x, _I.y, _I.z, _g, _kf, _Lv, _Lh);
    Eigen::VectorXd W_f = dis.first;
    Eigen::VectorXd W_pred = W_f;
    Eigen::Vector3d torques_true = dis.second;
    ...
    for (int i = 0; i < _Np; ++i) {
        ...
        //_dis_state_est.insert(_dis_state_est.end(), _dis_state.data(), _dis_state.data() + _dis_state.size());
    }
    std::pair<std::vector<double>, double> sol = mpc.solve(current_state, ref_state, _dis_state_est, _grasp_flag);
    MPC = sol.first;
    ...
    //_current.tau_x = torques_true[0];
    _previous_e.tau_x = _e.tau_x;   
    _e.tau_x = MPC[2] + W_f[3] - _current.tau_x; 
    ...
    if (publish_counter >= 10) {
       std::vector<double> _debug = {_current.p_global, _current.q_global, _current.r_global};
       _current_drone_twist_msg1.data = _debug;       
       _current_drone_twist_pub.publish(_current_drone_twist_msg1);
       publish_counter = 0;
    }
    
    _wrench_msg.data.resize(W_f.size());
    for (int i = 0; i < W_f.size(); ++i) {
        _wrench_msg.data[i] = W_f[i];
    }      
    _wrench_pub.publish(_wrench_msg); 
       
    _end_effector_msg.x = end_effector_position[0];
    _end_effector_msg.y = end_effector_position[1];
    _end_effector_msg.z = end_effector_position[2];
    _end_effector_pub.publish(_end_effector_msg);
    ...
    ROS_INFO("Fx_est: %f", W_f[0]);
    ROS_INFO("Fy_est: %f", W_f[1]);
    ROS_INFO("Fz_est: %f", W_f[2]);
    ROS_INFO("tau_x_est: %f", W_f[3]);
    ROS_INFO("tau_y_est: %f", W_f[4]);
    ROS_INFO("tau_z_est: %f", W_f[5]);
    
    //ROS_INFO("tau_x_true: %f", torques_true[0]);
    //ROS_INFO("tau_y_true: %f", torques_true[1]);
    //ROS_INFO("tau_z_true: %f", torques_true[2]);
    ...
    //ROS_INFO("roll rate PX4: %f", roll_rate);
    //ROS_INFO("roll rate Gazebo: %f", _current.p);
    //ROS_INFO("pitch rate PX4: %f", pitch_rate);
    //ROS_INFO("pitch rate Gazebo: %f", _current.q);
    //ROS_INFO("yaw rate PX4: %f", yaw_rate);    
    //ROS_INFO("yaw rate Gazebo: %f", _current.r);
...
std::array<std::array<double, 3>, 3> FlightController::transposeMatrix(const std::array<std::array<double, 3>, 3>& mat) {
...
std::array<double, 3> FlightController::addVectors(
